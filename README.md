...
**どこでAIを使いたいかで、やる項目が決まります。**

| あなたの使い方 | やる項目 |
|---|---|
| Claude Desktop（チャットアプリ）で使いたい | **A** |
| ターミナルや VS Code の Claude Code で使いたい | **B** |
| **両方で使いたい** | **AとBの両方**（同じサブドメイン・同じAPIキーを2か所に入れるだけ） |
| Cursor で使いたい | **C** |
| Codex で使いたい | **D** |

※ 設定はアプリごとに別々に保存されます。片方に設定しても、もう片方には自動では現れません。
...
**すでに他の設定が書いてあるファイルだった場合**は、丸ごと上書きせず「差し込み」ます。`⌘+F` で `"mcpServers"` を検索し、`"mcpServers": {` の行の**すぐ下**に次のブロックを貼ってください（既存の行は1文字も変えなくてOK。**最後の `},` のカンマ**が、下に続く既存設定とのつなぎ目になります）。
...
**Windowsで「接続できない」となる場合**

Windows版では `"command": "npx"` がうまく起動しないことがあります。その場合は `command` と `args` を次の形に書き換えてください（`env` はそのまま）。

```json
"command": "cmd",
"args": ["/c", "npx", "-y", "connectedone-mcp"],
```
...
```
claude mcp add connectedone -s user -e CONNECTEDONE_SITE_URL=https://あなたのサブドメイン.connected-one.world -e CONNECTEDONE_API_KEY=取得したAPIキー -- npx -y connectedone-mcp
```
...
```
codex mcp add connectedone --env CONNECTEDONE_SITE_URL=https://あなたのサブドメイン.connected-one.world --env CONNECTEDONE_API_KEY=取得したAPIキー -- npx -y connectedone-mcp
```

※ このサーバーは標準のMCP仕様なので、Gemini CLI / Cline / VS Code など他のMCP対応ツールでも使えます。各ツールのMCP設定に、上記 A と同じ `command` / `args` / `env` を書いてください。

---

## 動作確認

Claude を起動し直して、**あなたのサイト名を入れて**こう話しかけてください。

```
◯◯の購読者リストを一覧して
```

例: 「**cfbooks の購読者リストを一覧して**」
...
| 「git が必要」のようなエラーが出る | Git未導入のパソコン（主にWindows）で起きることがあります。<https://git-scm.com/download/win> からインストール後、再実行してください |
| Windowsで接続できない | ステップ③Aの「Windowsで接続できない場合」の `cmd /c` 形式に書き換えてください |
